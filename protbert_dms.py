...
DMS_DIR = "./mutation_effects/DMS_ProteinGym_substitutions"
SAVE_DIR = "./mutation_effects/Protbert_DMS_results"
STRUCT_PATH = "./mutation_effects/DMS_structure.pkl"
...
model = load_pretrain_model(model_path='./checkpoints/proteinbert_node.pt', model=net)
...
def predict_batch(seqs, infos, ca_tensor):
    ...
    for s in tqdm(range(0, len(legal), step), leave=False, desc="批次"):
        ...
        annotation = torch.zeros(B, 1, device=device)

        logits = model(seq=feats_masked, mask=mask, annotation=annotation) # (B,L,22)
