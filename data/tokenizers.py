...
class ProtacTokenizer:
    def __init__(
        ...
        self.poi_seq = self.data['poi_seq']
        self.e3_ligase_seq = self.data['e3_ligase_seq']

        print(f"Dataset Length is {len(self.poi_seq)}")
    ...
    def tokenize(self):
        ...
        # input_ids = list(map(self.encode, self.sequences))
        poi_input_ids = []
        for i in tqdm(range(len(self.poi_seq))):
            poi_input_ids.append(self.encode(self.poi_seq[i]))
        # Generate masks (1 for non-padding, 0 for padding)
        print("Generate the masks...")
        poi_masks = [[1 if token != self.amino2dict['<PAD>'] else 0 for token in seq] for seq in poi_input_ids]
        
        e3_ligase_input_ids = []
        for i in tqdm(range(len(self.e3_ligase_seq))):
            e3_ligase_input_ids.append(self.encode(self.e3_ligase_seq[i]))
        # Generate masks (1 for non-padding, 0 for padding)
        print("Generate the masks...")
        e3_ligase_masks = [[1 if token != self.amino2dict['<PAD>'] else 0 for token in seq] for seq in e3_ligase_input_ids]
        
        return poi_input_ids, poi_masks, e3_ligase_input_ids, e3_ligase_masks
