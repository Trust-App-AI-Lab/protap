...
class DrugCLIP(UnicoreTask):
    ...
    def valid_step(self, sample, model, loss, test=False):
        ...
        with torch.no_grad():
            loss, sample_size, logging_output = loss(model, sample)
            print(loss, sample_size, logging_output)
        return loss, sample_size, logging_output
    
    def train_model(self, model, optimizer, loss_fn, args):
        """Train the DrugCLIP model.
        
        Args:
            model: The DrugCLIP model
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            shuffle=False,  # Since we use SortDataset with shuffle for training
        ...
        for epoch in range(args.max_epoch):
            ...
            for batch_idx, sample in enumerate(train_pbar):
                ...
                # Training step using the existing train_step method
                loss, sample_size, log_output = self.train_step(
                    sample, model, loss_fn, optimizer, num_updates
                )
            ...
            if (epoch + 1) % args.validate_interval == 0:
                ...
                # First, do validation step
                for valid_batch in valid_loader:
                    valid_batch = unicore.utils.move_to_cuda(valid_batch)
                    loss, sample_size, log_output = self.valid_step(valid_batch, model, loss_fn)
                    valid_loss += loss.item()
                
                # Then evaluate on the test task (DUDE or PCBA)
                if args.test_task.upper() == "DUDE":
                    ...
                    auc, bedroc, ef, re, _, _ = self.test_dude(
                        test_loader, 
                        test_loader,  # Same loader since data is combined
                        model
                    )
                else:  # PCBA
                    auc, bedroc, ef, re = self.test_pcba(
                        test_loader,
                        test_loader,  # Same loader since data is combined
                        model
                    )
                ...
                if bedroc > best_valid_bedroc:
                    best_valid_bedroc = bedroc
                    patience_counter = 0
                    ...
                    checkpoint = {
                        ...
                        'best_bedroc': best_valid_bedroc,
                    }
                    torch.save(checkpoint, f"{args.save_dir}/checkpoint_best.pt")
                else:
                    patience_counter += 1
            ...
            if (epoch + 1) % args.save_interval == 0:
                ...
                torch.save(checkpoint, f"{args.save_dir}/checkpoint_last.pt")
        
        print(f"Training finished. Best validation BEDROC: {best_valid_bedroc:.4f}")
        return model
