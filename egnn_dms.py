...
SAVE_DIR = "./data/mutation_effects/EGNN_DMS_results"
STRUCT_PATH = "./data/mutation_effects/DMS_structure.pkl"
# Model path.
MODEL_PATH = "./checkpoints/egnn_node.pt"
...
model = load_pretrain_model(model_path=MODEL_PATH, model=net).to(device)
...
def predict_batch(seqs, infos, ca_tensor):
    ...
    for s in tqdm(range(0, len(legal), step), leave=False, desc="批次"):
        ...
        logits = model(feats=feats_masked, coors=coors, mask=mask)[0]   # (B,L,22)
